...
**Measured by Mate (matebook, 2026-09-01 22:07Z) on its own live interactive pane w2:p1 after Route A:** `pane.report_agent blocked` applied (seq 1315→1316) and **held for 12+ s against a contradicting screen scrape** (OSC title still spinning, `osc_title_working` priority 1100) until `working` was re-reported (seq 1319). So authored state wins on a live pane, not only on a fresh `claude -p` pane, and the old `herdr:claude` ownership did not survive the Claude restart. Also: `agent explain` reported `rule: osc_title_working` **while the authored blocked state was in force** — explain narrates the screen-detection path unconditionally, blind to authored state in both directions (an OpenCode pane shows `screen_detection_skip_reason: full_lifecycle_hook_authority` instead). Write-up: `~/Work/.herd/results/route-a-postcheck.mate.md` on matebook.
